...
private:
    ...
    std::unordered_map<Price, std::vector<std::pair<OfferID, Count>>> asks_;
...
public:
    Level2();
    auto add_order(unsigned int quantity, Price price) -> OfferID;
    ...
    bool get_offer_by_id(OfferID id, std::pair<OfferID, Count>& offerid) ;
